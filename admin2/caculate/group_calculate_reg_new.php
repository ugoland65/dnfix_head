...
$pageName = "group_calculate_reg";
		...
		$cal_data = wepix_fetch_array(wepix_query_error("select * from "._DB_CALCULATE." where CAL_BKG_IDX = ".$_bkg_idx." order by CAL_IDX desc"));
...
<tr align="center">
	...
	<td ><button type="button" id="" class="btnstyle1 btnstyle1-primary btnstyle1-sm" onclick="bookingInfo('<?=$_ary_bkp_idx[$i]?>')" >INFO</button></td>
</tr>
<?
	} //for END
?>
...
<?
	...
	$food_data_count = wepix_counter2(_DB_BUY_PRODUCT_TRAVEL, $search_sql, "BP_PD_IDX");
...
<?
	...
	$_row_num = 0;
	$buydata = wepix_query_error("select * from "._DB_BUY_PRODUCT_TRAVEL."  ".$search_sql ." GROUP BY BP_PD_IDX");
...
<?
	...
	$tour_data_count = wepix_counter2(_DB_BUY_PRODUCT_TRAVEL, $search_sql_tour, "BP_PD_IDX");
...
<?
	...
	$_row_num = 0;
	$buydata = wepix_query_error("select * from "._DB_BUY_PRODUCT_TRAVEL."  ".$search_sql_tour ." GROUP BY BP_PD_IDX");
			while($pdlist = wepix_fetch_array($buydata)){
...
<table class="table-style" >
	...
	<tr>
		...
		<table class="table-style" >
			<th class="text-center this-team-num">팀번호</th>
			<th class="text-center this-team-name2">Name</th>
			<th class="text-center" style="width:50px;">화폐</th>
			<th class="text-center" style="width:100px;">환율</th>
			<th class="text-center this-list-price">입금</th>
			<th class="text-center this-list-price">환율 적용금</th>
...
<?
	...
	if($_view_group_type == 'HM' || $_view_group_type == 'FA'){
		...
		$_view_bkp_total_tm_dollar = $_view_total_tm; //추가 T/M 달러
...
?>
	...
	<table class="table-style">
		<tr>
			...
			<td style="padding:0; border:none; width:5px;"></td>
			<th class="text-center" colspan='6'>BRD</th>
			<td style="padding:0; border:none; width:5px;"></td>
		...
		<tr>
			...
			<div class="page-btn-wrap">
				<ul class="page-btn-left">
					<button type="button" id="" class="btnstyle1 btnstyle1-inverse btnstyle1-lg" onclick="location.href='<?=_A_PATH_BOOKING_GROUP_LIST?>'" > 
				...
				<?if($_view_cal_state <= 1){?>
					<button type="button" id="" style='background:#ff9e96; color:white;' class="btnstyle1 btnstyle1 btnstyle1-lg" onclick="doCalCuSubmit('<?=$_cal_mode?>','temporary');" > 
						<i class="far fa-check-circle"></i>
						임시 저장
					</button>
				<?}?>
					<button type="button" id="" class="btnstyle1 btnstyle1-primary btnstyle1-lg" onclick="doCalCuSubmit('<?=$_cal_mode?>','save');" > 
						<i class="far fa-check-circle"></i>
						정산서 저장
					</button>
				</ul>
...
function doCalCuSubmit(cal_mode,mode){
