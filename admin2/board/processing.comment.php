...
if( $_action_mode == "commWrite" ){
	...
	if( $_comm_mode == "IG" OR $_comm_mode == "IG2" ){
		$_comm_name = $_comm_write_name;
		$_comment_admin = "N";
	}
	...
	if( $_reply_mode == "on" AND $_reply_key ){
		...
		//원글 불러오기
		$reply_data = wepix_fetch_array(wepix_query_error("select HEADNUM, DEPTH from "._DB_BOARD_COMMENT." WHERE COMMENT_IDX = '".$_reply_key."' "));
...
/*
		...
		$max_no = wepix_fetch_array(wepix_query_error("select max(HEADNUM) from "._DB_BOARD_COMMENT." where BOARD_UID = '".$_b_key."' AND HEADNUM > ".$first_headnum." AND HEADNUM < ".$last_headnum." "));
		$temp_headnum = ( $max_no[0] ) ?  $max_no[0] + 1 : $reply_data[HEADNUM] + 1;
*/
		//내림차순 정렬시 (등록순 위로)
		$last_headnum = $first_headnum - 100;
		$min_no = wepix_fetch_array(wepix_query_error("select min(HEADNUM) from "._DB_BOARD_COMMENT." where BOARD_UID = '".$_b_key."' AND HEADNUM > ".$last_headnum." AND HEADNUM < ".$first_headnum." "));
	
/*
			echo "|Erorr|".$first_headnum."/".$last_headnum."/".$min_no[0]."|";
			exit;
*/	
		$temp_headnum = ( $min_no[0] ) ?  $min_no[0] - 1 : $reply_data[HEADNUM] - 1;
...
/*
		...
		exit;
*/
		wepix_query_error("update "._DB_BOARD_COMMENT." set COMMENT_REPLY = COMMENT_REPLY + 1 WHERE COMMENT_IDX = '".$_reply_key."' " );
		...
		if( $temp_depth == 1 && $_mom_headnum ){
			...
			$min_no = wepix_fetch_array(wepix_query_error("select min(HEADNUM) from "._DB_BOARD_COMMENT." where BOARD_UID = '".$_b_key."' AND HEADNUM > 1000 AND DEPTH = 0 "));
	...
	$query = "insert "._DB_BOARD_COMMENT." set
    ...
    // 코멘트 총 갯수 넣기
    wepix_query_error("update "._DB_BOARD." set BOARD_COMMENT = BOARD_COMMENT + 1 where UID = '".$_b_key."' " );
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$comment_data = wepix_fetch_array(wepix_query_error("select COMMENT_IDX from "._DB_BOARD_COMMENT." WHERE COMMENT_IDX = '".$_c_key."' "));

	//데이터가 존재 할경우만
	if( $comment_data[COMMENT_IDX] ){

		wepix_query_error(" delete from "._DB_BOARD_COMMENT." where COMMENT_IDX = '".$_c_key."' ");

		// 코멘트 총 갯수 감소
		//ttt_query_error("update "._DB_BOARD." set board_comment = board_comment - 1 where uid = '".$_board_key."' " );

		//코멘트 다시 카운팅
		$comment_count = wepix_counter(_DB_BOARD_COMMENT, " WHERE BOARD_UID='".$_b_key."' ");
		wepix_query_error("update "._DB_BOARD." set BOARD_COMMENT='".$comment_count."' where UID = '".$_b_key."' " );
