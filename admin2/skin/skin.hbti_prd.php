...
$viewData = $productController->hbtiPrdIndex();
	...
	<div class="head-right-fixed-wrap">
		...
		<div class="m-t-7">
			...
			<ul class="m-t-5">
				...
				<select name="s_hbti_type" id="s_hbti_type" >
					<option value="">전체 HBTI</option>
					<?
					foreach($hbtiTypes as $hbtiType){
					?>
					<option value="<?=$hbtiType?>" <? if( ($_GET['s_hbti_type'] ?? '') == $hbtiType ) echo "selected";?>><?=$hbtiType?></option>
					<? } ?>
				</select>
		...
		<div class="hbti-count-wrap">
			<div>
				<?php foreach($viewData['hbtiCount']['counts'] as $key => $value){ ?>
					
					<a href="/ad/prd/hbti_prd/?s_hbti_type=<?=$key?>">
					<p>
						<span><b><?=$key?></b></span>
						<span><?=$value?> 개</span>
					</p>
...
<div id="contents_body">
	...
	<table class="table-style m-t-6">	
		...
		<tr align="center" id="trid_<?=$list['CD_IDX']?>" class="<?=$_tr_class?>">
			...
			<td class="">
				<div>
					<ul><a href="/ad/prd/hbti_prd/?s_hbti_type=<?=$list['cd_hbti']?>"><?=$list['cd_hbti']?></a></ul>
				</div>
			</td>
			<td class="">
				<?=$list['cd_godo_code']?>
...
<script>
	$(function(){
        ...
        $("#s_brand, #s_kind_code, #s_national, #s_tier, #s_hbti_type").on('focusout', function() {
        ...
        $("#searchBtn").on('click',function(){
            ...
            // 페이지 이동
            location.href = '/ad/prd/hbti_prd' + (queryString ? '?' + queryString : '');
        });
    });
