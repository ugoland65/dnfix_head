		...
		<div class="ost-head">
			...
			<? if( $oo_data['oo_idx'] ){ ?>
				...
				<? if( $_json_oo_stock['state'] == "in" ){ ?>
					<span style="font-size:12px; font-weight:500;">재고일괄등록 완료 ( <?=date ("y.m.d <b>H:i</b>", strtotime($_json_oo_stock['reg']['date']))?> )</span>
				<? } ?>

				<? if( $oo_data['oo_form_idx'] ){ ?>
				...
				<div id="order_sheet_info">
					<div class="order-sheet-info-wrap">
						...
						<ul class="m-t-15">전체 금액 : 
							<b><span id="oprice_allsum"><?=number_format($oo_data['oo_sum_price'],2)?></span></b>
							<? if( $oo_data['oo_sum_currency'] ){ ?><?=$oo_data['oo_sum_currency']?><? } ?>
						</ul>
						...
						<ul class="m-t-5">원화 전환 : <b><span id=""><?=number_format($oo_data['oo_sum_price'] * $oo_data['oo_sum_exchange_rate'],2)?></span></b></ul>
						<? } ?>
						<ul class="m-t-5">전체 상품 : <b><span id="oprice_sum_goods"><?=number_format($oo_data['oo_sum_goods'])?></span></b></ul>
						<ul class="m-t-5">전체 수량 : <b><span id="oprice_sum_qty"><?=number_format($oo_data['oo_sum_qty'])?></span></b></ul>
						<ul class="m-t-5">전체 무게 : <b><span id="oprice_sum_weight"><?=number_format($oo_data['oo_sum_weight'])?>g</span></b></ul>
						<ul class="m-t-5">전체 CBM : <b><span id="oprice_sum_cbm"><?=number_format($oo_data['oo_sum_cbm'],2)?></span></b></ul>
					</div>
