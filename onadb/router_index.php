...
use App\Controllers\Onadb\CommentController;
use App\Controllers\Onadb\BrandController;
...
try {
    ...
    $router->get('/pv/{idx}', ProductController::class, 'productDetail'); // 상품 상세페이지
    $router->post('/prd-comment', ProductController::class, 'productCommentReg'); // 상품 평점 등록
    $router->get('/tier/{tier}', ProductController::class, 'productTierList'); // 티어정보
    $router->get('/brand/{idx}', ProductController::class, 'brandProductList'); // 브랜드 상품

    $router->get('/brandlist', BrandController::class, 'brandList'); // 브랜드 목록

    $router->get('/u/@{nickname}', CommentController::class, 'userCommentMainPage'); // 사용자 코멘트 메인
