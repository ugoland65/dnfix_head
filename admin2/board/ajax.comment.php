<?
	...
	//댓글쿼리
	$comment_query = "where BOARD_UID ='".$_b_key."' ";
	$total_count = wepix_counter(_DB_BOARD_COMMENT, $comment_query);
	...
	$comment_result = wepix_query_error("select * from "._DB_BOARD_COMMENT." ".$comment_query."order by HEADNUM desc limit ".$from_record.", ".$list_num." ");

	$_view_paging = publicAjaxPaging($_pn, $total_page, $list_num, $page_num, 'showComment','');

?>
<STYLE TYPE="text/css">
.bo-view-comment-wrap{ padding:10px; box-sizing:border-box;  }
.comm-ajax{ width:700px; }

.bo-view-comment-write{ border:1px solid #dedede; background-color:#f5f8f9; box-sizing:border-box; padding:10px; }
...
.comment-list-wrap.bs{ background-color:#fff; }
.comment-list-wrap.del{ background-color:#eee; }

.comment-list-wrap ul.col-3{ display:table-cell; }
.comment-list-depth-gap{ width:30px; text-align:right; }
.comment-list-m-icon{ width:66px;  box-sizing:border-box; padding:5px; display:none !important; }
...
.comment-list-body{ padding:13px 0 0 15px; vertical-align:top; }
...
.bo-view-comment-list-filter{ margin-left:15px; }
.bo-view-comment-list-filter .title{ height:30px; line-height:30px; }
.bo-view-comment-list-filter .wrap{
	width:450px; border:1px solid #dedede;
}

.comment-write-wrap-right{ width:450px; border:1px solid #dedede; background-color:#f5f8f9; box-sizing:border-box; padding:10px; margin:10px 0 0 15px; }
...
<div class="bo-view-comment-wrap display-table">
	<ul class="comm-ajax display-table-cell v-align-t">
...
<?
	while($comment_list = wepix_fetch_array($comment_result)){
		...
		if( $comment_list[COMMENT_MODE] == "IG" OR $comment_list[COMMENT_MODE] == "IG2" ){
			$_view_comm_id = "가상 : ".$comment_list[COMMENT_ID];
		}else{
			$_view_comm_id = $comment_list[COMMENT_ID];
		}
		
		$_view_comm_body = nl2br($comment_list[COMMENT_BODY]);
		$_view_comm_ip_show = $comment_list[COMMENT_IP_SHOW];

	$_line_color_class = "";
	if( $comment_list[COMMENT_MODE] == "IG" OR $comment_list[COMMENT_MODE] == "IG2" ){
	}elseif( $comment_list[COMMENT_MODE] == "BS" ){
		$_line_color_class = "bs";
	}

	if( $comment_list[COMMENT_SHOW] == "del" OR $comment_list[COMMENT_SHOW] == "delview" ){
		$_line_color_class = "del";
	}

?>
			<div class="comment-list-wrap <?=$_line_color_class?>">
<?
...
if( $comment_list[COMMENT_SHOW] == "del" OR $comment_list[COMMENT_SHOW] == "delview" ){
?>
<b style="color:#ff0000;">[삭제]</b>
<? } ?>
							<!-- <span class="clb-wrap-info-level">(<?=$comment_list[comm_level]?>)</span> -->
							<span class="clb-wrap-info-name">(<?=$comment_list[COMMENT_IDX]?>) <b><?=$comment_list[COMMENT_NAME]?></b></span> | 
							<span class="clb-wrap-info-id"><?=$_view_comm_id?></span> |
							<span class="clb-wrap-info-id"><?=$_view_comm_ip_show?></span> |
					...
					<ul>
						<div class="btn-group radio-form" data-toggle="buttons">
							...
							<label class="btn btn-default " onclick="bo_mode_ig('on', 'reply', '<?=$comment_list[COMMENT_IDX]?>');">
								<input type="radio" name="bo_mode_reply_<?=$comment_list[COMMENT_IDX]?>" id="bo_mode1_reply_<?=$comment_list[COMMENT_IDX]?>" value="IG" autocomplete="off" > 가상(회원)
							</label>
							<label class="btn btn-default " onclick="bo_mode_ig('on', 'reply', '<?=$comment_list[COMMENT_IDX]?>');">
								<input type="radio" name="bo_mode_reply_<?=$comment_list[COMMENT_IDX]?>" id="bo_mode1_reply_<?=$comment_list[COMMENT_IDX]?>" value="IG2" autocomplete="off" > 가상(비회원)
							</label>
						</div>
					</ul>
					<ul id="comm_name_wrap_reply_<?=$comment_list[COMMENT_IDX]?>" class="m-t-5 display-none">
						<input type="text" name="comm_name" id="comm_write_name_reply_<?=$comment_list[COMMENT_IDX]?>"  value="" style="width:100px !important;" placeholder="작성자명">
						<input type="text" name="comm_ip_show" id="comm_ip_show_reply_<?=$comment_list[COMMENT_IDX]?>"  value="" style="width:100px !important;" placeholder="노출 IP">
...
<? } //while END?>
	...
	<ul class="display-table-cell v-align-t">

		<div class="bo-view-comment-list-filter">
			<ul class="title"><i class="fas fa-filter"></i> Filter</ul>
			<ul class="wrap">
			...
			<div class="form">
				<ul>
					<div class="btn-group radio-form" data-toggle="buttons">
						...
						<label class="btn btn-default " onclick="bo_mode_ig('on');">
							<input type="radio" name="bo_mode" id="bo_mode3" value="IG2" autocomplete="off" > 가상(비회원)
						</label>
					</div>
				</ul>
				<ul id="comm_name_wrap" class="display-none">
					...
					<input type="text" name="comm_ip_show" id="comm_ip_show"  value="" style="width:100px !important;" placeholder="노출 IP">
					<!-- <input type="text" name="comm_level" id="comm_level" value="<?=$_view_board_site_name?>" style="width:30px !important; text-align:center !important;"> 레벨 -->
				</ul>
				<ul>
					작성일 : 
...
function commentSubmit(mode, key){
	...
	if( mode == "reply" ){
		...
		var comm_ip_show = $('#comm_ip_show_reply_'+key).val();
	}else{
		...
		var comm_ip_show = $('#comm_ip_show').val();
	}
