...
include "../lib/inc_common.php";
	...
	$hotel_where = "where HOT_IDX > 0 ";

	$total_count = wepix_counter(_DB_HOTEL, $hotel_where);
	...
	$hotel_query = "select * from "._DB_HOTEL." ".$hotel_where." order by HOT_IDX desc limit ".$from_record.", ".$list_num;
	$hotel_result = wepix_query_error($hotel_query);

	$page_link_text = _A_PATH_PRODUCT_HOTEL_LIST."?pn=";
...
<div id="contents_head">
	...
	<div id="head_write_btn">
		<button type="button" id="" class="btnstyle1 btnstyle1-danger btnstyle1-lg" onclick="location.href='<?=_A_PATH_PRODUCT_HOTEL_REG?>'" > 
	...
	<div id="contents_body_wrap">
		...
		<div class="list-top-btn-wrap">
			<ul class="list-top-total">
				<span class="count">등록된 호텔 : <b><?=$total_count?></b></span>
			</ul>
...
<?
				while($hotel_list = wepix_fetch_array($hotel_result)){
					$_view2_hotel_active  = ( $hotel_list[HOT_VIEW]=="Y" ) ? "노출" : "비노출";
					$_view2_roomtype_count = wepix_counter(_DB_HOTEL_ROOM_TYPE, "where ROC_HOT_IDX = '".$hotel_list[HOT_IDX]."' ");
?>
					<tr align="center" id="trid_<?=$hotel_list[HOT_IDX]?>" bgcolor="<?=$trcolor?>">
						<td class="pd-checkbox" ><input type="checkbox" name="key_check[]" value="<?=$hotel_list[HOT_IDX]?>" ></td>
						<td><?=$hotel_list[HOT_IDX]?></td>
						<td><?=$bva_tr_area_name_ko[$hotel_list[HOT_AREA]]?></td>
						<td class="pd-name">
							<a onclick="productModify('<?= $hotel_list[HOT_IDX]?>');">
								<b><?=$hotel_list[HOT_NAME]?></b><br>
								<?=$hotel_list[HOT_FULL_NAME]?>
							</a>
						</td>
						<td><?=$_view2_roomtype_count?></td>
...
function productModify(key){
	location.href = "<?=_A_PATH_PRODUCT_HOTEL_REG?>?key="+key+"&mode=modify";
}
