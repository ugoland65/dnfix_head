...
<div id="contents_body">
	<div id="contents_body_wrap">
		...
		<? if( $_idx ){ ?>
			<input type="hidden" name="a_mode" value="work_manual_modify" >
			<input type="hidden" name="idx" value="<?=$_idx?>" >
		<? }else{ ?>
			<input type="hidden" name="a_mode" value="work_manual_reg" >
		<? } ?>
				...
				<td>
					...
					<?
						for ($i=0; $i<count($_work_manual_cate); $i++){
							$selected = ( $data['category'] == $_work_manual_cate[$i]['name'] ) ? "selected" : "";
							echo "<option value=\"".$_work_manual_cate[$i]['name']."\" ".$selected.">".$_work_manual_cate[$i]['name']."</option>";
					} 
					?>
					</select>
				</td>
				...
				<td>
					...
					<div class="file-list-wrap">
						...
						<div class="file-list">
							...
							<input type="hidden" name="old_work_manual_file[]" value="<?=$_file_data['file_name'][$i]?>" >
						</div>
						<? } ?>
					</div>
					</div>
					...
					<div class="file-list-wrap" id="file_list_wrap">
						<div class="file-list">
							<input name="work_manual_file[]" type="file" >
						</div>
					</div>
					<div>
						<button type="button" id="" class="btnstyle1 btnstyle1-success btnstyle1-sm" onclick="workManualReg.addFile();" >첨부파일 추가</button>
					</div>
				</td>
			 </tr>
		...
		<div class="submitBtnWrap">
				...
				<button type="button" id="" class="btnstyle1 btnstyle1-inverse btnstyle1-lg" onclick="location.href='/ad/staff/work_manual'" > 
					<i class="fas fa-arrow-left"></i> 목록
				</button>

				<button type="button" id="" class="btnstyle1 btnstyle1-primary btnstyle1-lg" onclick="workManualReg.save(this);" > 
		...
		save : function( obj ) {
			...
			$.ajax({
				...
				success: function(res){
					if ( res.success == true ){
						alert("저장되었습니다.");
						location.href='/ad/staff/work_manual_view/'+ res.key;
					}else{
