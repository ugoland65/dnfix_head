						...
						<td>
                            <div id="change_price" class="change-price">
                                ...
                                <ul>
									...
									<li>
									<li><input type="text" name='change_price_price[]' class="price" placeholder="금액" value="<?=number_format($change_price['price'] ?? 0,2)?>"><li>
									<li><input type="text" name='change_price_body[]' class= 'change-price-body' value="<?=$change_price['body'] ?? ''?>" placeholder="사유"><li>
			...
			<td>
				...
				<table class="table-style width-full">
					...
					<tr>
						<th>배송비 결제기한</th>
						<td>
							<div class="calendar-input" style="display:inline-block;"><input type="text" name="expressApprovalPayment_date"  id="expressApprovalPayment_date" value="<?=$orderSheetInfo['oo_approval_date']['express']['approval']['date'] ?? ''?>" autocomplete="off"></div>
...
var orderSheetReg = (function() {
    ...
    function orderSheetState(obj, state, idx) {
        ...
        var payload = { action_mode: 'order_sheet_state', idx: idx, state: state };
    ...
    function fileDel(obj, mode, idx, filename) {
        ...
        $.ajax({
            url: "/admin/order/sheet/action",
            data: { "action_mode": "orderSheetFileDelete", "idx": idx, "smode": mode, "filename": realFilename },
            type: "POST",
            dataType: "json",
