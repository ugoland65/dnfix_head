...
*/
		if( $_s_text ){
			...
			if( $_serch_query ){ $_serch_query .= "AND"; }else{ $_serch_query .= "WHERE"; }

			if (preg_match("/[a-zA-Z]/", $_s_text)){
				$_serch_query .= " ( ";
				$_serch_query .= " INSTR(LOWER(CD_NAME), LOWER('".$_s_text."')) ";
				$_serch_query .= " or INSTR(replace(CD_NAME,' ',''), LOWER('".$_s_text."')) ";
				$_serch_query .= " or INSTR(LOWER(CD_SEARCH_TERM), LOWER('".$_s_text."')) ";
				$_serch_query .= " or INSTR(LOWER(CD_NAME_OG), LOWER('".$_s_text."')) ";
				$_serch_query .= " or INSTR(cd_code_fn, '".$_s_text."') ";
				//$_serch_query .= " or STOCK.ps_idx = '".$_s_text."' ";
				$_serch_query .= " ) ";
			}else{
				//$_serch_query .= " and (INSTR(LOWER(CD_NAME), '".$_s_text."') or INSTR(LOWER(CD_SEARCH_TERM),  '".$_s_text."') or INSTR(LOWER(CD_NAME_OG), '".$_s_text."') )";
				$_serch_query .= " ( ";
				$_serch_query .= " INSTR(CD_NAME, '".$_s_text."') ";
				$_serch_query .= " or INSTR(replace(CD_NAME,' ',''), '".$_s_text."') ";
				$_serch_query .= " or INSTR(CD_SEARCH_TERM, '".$_s_text."') ";
				$_serch_query .= " or INSTR(CD_NAME_OG, '".$_s_text."') ";
				$_serch_query .= " or INSTR(cd_code_fn, '".$_s_text."') ";
				$_serch_query .= " ) ";
			}
		}
		...
		if ( $_s_brand ){
			if( $_serch_query ){ $_serch_query .= "AND"; }else{ $_serch_query .= "WHERE"; }
			$_serch_query .= " ( CD_BRAND_IDX = '".$_s_brand."' OR CD_BRAND2_IDX = '".$_s_brand."' ) ";
			//$_serch_query .= " CONCAT ( CD_BRAND_IDX, CD_BRAND2_IDX ) REGEXP  '".$_s_brand."' ";
		}

		if( $_s_kind_code){
			if( $_serch_query ){ $_serch_query .= "AND"; }else{ $_serch_query .= "WHERE"; }
			$_serch_query .= " CD_KIND_CODE = '".$_s_kind_code."' ";
		}
...
*/
	...
	if( $_sort_kind == "hit" ) {
		...
		$_sort_text = "CD_RELEASE_DATE desc";
	}else{
		$_sort_text = "CD_IDX desc";
	}

	$_join_sql = " PRD left join prd_stock STOCK ON (STOCK.ps_prd_idx = PRD.CD_IDX) ";
					...
					<table class="table-list">
						...
						while($list = wepix_fetch_array($result)){
							...
							if( $stock_data[ps_stock] > 0 ){
								$_stock_qty = $stock_data[ps_stock];
							}

							$_view_brand_name = $brand_data[BD_NAME];
							$_view_brand2_name = $brand2_data[BD_NAME];
						...
						<tr bgcolor="<?=$_trcolor ?>">
							...
							<td>
								<a href="comparison_list.php?s_active=on&s_brand=<?=$list[CD_BRAND_IDX]?>"><?=$_view_brand_name?></a><br>
								<? if( $list[CD_BRAND2_IDX] ){ ?><a href="comparison_list.php?s_active=on&s_brand=<?=$list[CD_BRAND2_IDX]?>"><?=$_view_brand2_name?></a><br><? } ?>
...
<? } ?>
			...
			<ul class="display-table-cell width-300 v-align-top">
				<div id="list_box_layout2_filter_wrap">
					<form name='search_form' id='search_form'  method='get' action="comparison_list.php">
