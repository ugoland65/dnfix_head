<?
	...
	$_board_grade = securityVal($bo_grade);
	$_board_subject = securityVal($board_subject);
	$_board_category = securityVal($board_category);
	$_board_mode = securityVal($bo_mode);
	$_board_witer_name = securityVal($board_witer_name);
	$_board_date_modify = securityVal($board_date_modify);
	$_board_date_day = securityVal($board_date_day);
	$_board_date_h = securityVal($board_date_h);
	...
	$_board_pd_idx = securityVal($bo_pd_idx);
	$_ary_relation_pd_idx = securityVal($relation_pd_idx);
	$_ary_relation_pd_idx_del = securityVal($relation_pd_idx_del);
	$_board_link_name = securityVal($bo_link_name);
	$_board_link_url = securityVal($bo_link_url);
	$_board_ip_show = securityVal($board_ip_show);

	if($uploadImageFileName!=""){
		$_uploadImageFileName = securityVal($uploadImageFileName);
	}

	$_idxArrayText = securityVal($idxArrayText);

//함수
//------------------------------------------------------------------------------------------------------------------
// 게시물 삭제 함수
function boardDel( $b_code, $b_key ){ 

    global $connect, $_db_board_a_config, $_db_file, $_db_comparison,$_show_bc_product_active, $_show_bc_product_mode, $_show_bc_grade_active;
...
if( $_action_mode == "new" ){
	...
	$_board_name = $_ad_nick;
	$_board_admin = "Y";

	//가상글 일때
	if( $_board_mode == "IG" OR $_board_mode == "IG2" ){
		$_board_name = $_board_witer_name;
		$_board_admin = "N";
	}

	//if (!isset($_board_name)){
	if ( !$_board_name ){
		if( $_ajax_mode == "on" ){
	...
	//이동할 폴더가 없을경우 생성한다
	if(!is_dir($_inst_img_move_dir)){
		//mkdir($_inst_img_move_dir,0777,true);
		@mkdir($_inst_img_move_dir, 0777);
		@chmod($_inst_img_move_dir, 0777);
   ...
   // 공지글 관련해서 번호 정하기, 헤드넘버 정하기
    if( $_board_mode == "NT" ) {
        $result = wepix_query_error("select min(headnum) from "._DB_BOARD." where headnum < 1000 and headnum > 1");
        $min_no = wepix_fetch_array($result);
        if($min_no[0]) $temp_headnum = $min_no[0] - 1; else $temp_headnum = 999;
    } else {
        $result = wepix_query_error("select min(headnum) from "._DB_BOARD." where headnum > 1000");
        $min_no = wepix_fetch_array($result);
	...
	//상품연동
	$_board_pd_name = "";
	if( $_show_bc_product_active == "Y" && $_board_pd_idx ){
		if( $_show_bc_product_mode == "comparison" ){ //가격비교 그룹일경우
	...
	if( $_show_bc_product_active == "Y" ){
		...
		if( $_show_bc_product_mode == "comparison" ){
			if( $_show_bc_grade_active == "Y" ){ //평점기능을 사용중일경우 
				if( $_board_pd_idx ){
					$pd_review_sum = wepix_fetch_array(wepix_query_error("select format(avg(BOARD_GRADE),1) as value1 from "._DB_BOARD." where BOARD_PD_IDX = '".$_board_pd_idx."' "));
					wepix_query_error("update "._DB_COMPARISON." set CD_REVIEW = CD_REVIEW + 1, CD_SCORE = '".$pd_review_sum[value1]."' where CD_IDX = '".$_board_pd_idx."' ");
				}
			}
		//상품연관 컨텐츠
		}elseif( $_show_bc_product_mode == "comparison_relation" ){
			for ($i=0; $i<count($_ary_relation_pd_idx); $i++){
				if( $_ary_relation_pd_idx[$i] > 0 ){
	...
	msg("",_A_PATH_BOARD_LIST."?b_code=".$_b_code);
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$result = boardDel( $_b_code, $_b_key );
/*
	$bo_data =  wepix_fetch_array(wepix_query_error("select * from "._DB_BOARD." where UID = '".$_b_key."'"));
	$_b_token = $bo_data[BOARD_TOKEN];
	$_b_pd_idx = $bo_data[BOARD_PD_IDX];
...
// 게시물 리스트 일괄 삭제
////////////////////////////////////////////////////////////////////////////////////////////////
}elseif( $_action_mode == "boardBatchDel" ){

	$idxArray = explode(',', $_idxArrayText);
	for($i=0;$i<count($idxArray);$i++){
		$result = boardDel( $_b_code, $idxArray[$i] );
	}
