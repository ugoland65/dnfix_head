...
for($i=0; $i<count($_ary2_mps_pd); $i++){ 
  if($_ary2_mps_pd[$i] != ''){
	...
	$pd_list = wepix_fetch_array(wepix_query_error("select * from BRAND_DB where BD_IDX = '".$_ary2_mps_pd[$i]."' "));

?>
	<tr align="center" id="<?=$_show_sort_id?>" bgcolor="<?=$trcolor?>">
		<td width="25px">
			<input type='hidden' name='mps_pd_idx[]' value='<?=$pd_list[BD_IDX]?>'>
		...
		<td width="80px"><?=$pd_list[BD_IDX]?></td>
		<td align="left"><a onclick="productModify('<?=$pd_list[BD_IDX]?>');"><b><?=$pd_list[BD_NAME]?></b></a></td>
		<td width="90px">
