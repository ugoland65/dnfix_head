		...
		<tbody>
			...
			?>
				...
				<tr bgcolor="<?= $_tr_color ?>" id="tr_<?= $_idx ?>">
					...
					<!-- 주문수량 -->
					<td style="width:55px;">
						<input type='text' name='cd_code2' id="unit_qty_<?= $_idx ?>" style="width:100%; font-size:15px; font-weight:bold; color:<? if (isset(${"_false_idx_" . $_idx}) && ${"_false_idx_" . $_idx} == "ok") {
																																					echo "#999";
																																				} else {
																																					echo "#021aff";
																																				} ?>;" value="<?= $save_qty[$_idx] ?? 0 ?>" onkeyUP="orderSheetDetail.qtyGogo('<?= $_idx ?>', '<?= $_oop_idx ?? '' ?>');">
					</td>
