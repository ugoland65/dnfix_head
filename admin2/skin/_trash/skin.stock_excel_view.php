				...
				<tbody>
					<tr>
						<td colspan="100%">
							...
							<div>
								<?php for ($i = 0; $i < $_error_result_count; $i++) { ?>
			...
			<tbody>
				...
				?>
					...
					<tr class="<?= $_tr_class ?>">
						...
						<td class="text-center">
							<?php if (($val['one_qty'] ?? 0) > 0) { ?><?= $val['one_qty'] ?><?php } ?>
						</td>
						<td class="text-center">
							<?php if (($val['set_qty'] ?? 0) > 0) { ?><?= $val['set_qty'] ?><?php } ?>
						</td>
	...
	var stockExcelView = (function() {
		...
		return {
								...
								$.ajax({
									...
									success: function(res) {
										if (res.status === "success" || res.success == true) {
											alert("삭제되었습니다.");
											GC.movePage("/admin/stock/stock_excel");
										} else {
			...
			dayStock: function(obj) {
				...
				var stockHistoryIdx = $("input[name='stock_history_idx']").val() || $("#sort_wrap").data("idx") || "";
