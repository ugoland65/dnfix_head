...
*/
	if( $_b_mode == "new" ){
		...
		$agc_result = wepix_query_error("select SD_NAME from SITE_DB where SD_ACTIVE = 'Y'");
		
		while($agc_data = wepix_fetch_array($agc_result)){
			$agc_company[] = $agc_data[SD_NAME];
		}
	...
	if( !$_show_board_mode ) $_show_board_mode = "BS";
					...
					<td colspan="3">
						<div class="btn-group radio-form" data-toggle="buttons">
							...
							<label class="btn btn-default active" onclick="bo_mode_ig('on');">
								<input type="radio" name="bo_mode" id="bo_mode1" value="IG" autocomplete="off" checked> 가상
							</label>
					...
					<td colspan="3">
						<div class="display-table">
							<ul id="board_witer_name_wrap" class="display-table-cell p-r-50" >
								작성자 : <input type='text' name='board_witer_name' id='board_witer_name' value="<?=$_view_witer_name?>" style="width:120px;" >
							</ul>
...
<? */ ?>
								...
								<label><input type="checkbox" name="board_date_modify" checked value="Y"> 작성일 지정</label>
							</ul>
							<ul class="display-table-cell p-l-50">
								조회수 : 
...
?>
				<tr>
					<th>썸네일 업로드</th>
					<td colspan="3"><input type="file" id="board_thumbnail" name="board_thumbnail" ></td>
				</tr>
<? } ?>
